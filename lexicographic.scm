...
;;
(define-library (okvstore lexicographic)

  (export pack unpack *null* struct:pack>Q struct:unpack>Q)
  ...
  (begin
    ...
    (define (struct:pack>Q integer)
      (let ((bytevector (make-bytevector 8 0)))
        (let loop ((index 0))
          (unless (= index 8)
            (set! bytevector index (bitwise-and
    ...
    (define (unpack-positive-integer bv code position)
      (let* ((n (- code 20))
        ...
        (let loop ((index 0))
          (unless (= index n)
            (set! sub (+ (- 8 n) index) (ref bv (+ position 1 index)))
            (loop (+ index 1))))
